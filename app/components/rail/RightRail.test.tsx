...
describe("RightRail", () => {
  ...
  it("falls back to highlightId when active is not a rail panel", () => {
    render(<RightRail active={null} onOpen={() => {}} highlightId="share" />);
    expect(screen.getByRole("button", { name: "Share" }).getAttribute("aria-current")).toBe("true");
  ...
  it("highlights the active rail panel even when a stale highlightId points elsewhere", () => {
    render(<RightRail active="inspect" onOpen={() => {}} highlightId="share" />);
    expect(screen.getByRole("button", { name: "Inspect" }).getAttribute("aria-current")).toBe("true");
